...
export default function MesEauPage() {
  ...
  return (
    <MainLayout
      ...
      breadcrumbs={[{ label: lang === 'ar' ? 'الماء، النور والتطهير' : 'Eau, Lumière & Égouts' }]}
    >
      <div className={`py-4 ${lang === 'ar' ? 'text-end font-arabic' : ''}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div>
            <h1 className="fw-bold section-title mb-1">
              <i className={`fas fa-tint ${lang === 'ar' ? 'ms-2' : 'me-2'}`}></i> {lang === 'ar' ? 'طلباتي — الماء، النور والتطهير' : 'Mes demandes — Eau, Lumière & Égouts'}
            </h1>
            <p className="text-muted mb-0">{lang === 'ar' ? 'سجل طلبات الربط بشبكات المياه والنور.' : 'Historique de vos demandes de raccordement et réclamations.'}</p>
          </div>
          <Link to="/demande-eau" className="btn btn-primary rounded-pill px-4 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="text-center py-5 card shadow-sm border-0 rounded-4">
            ...
            <Link to="/demande-eau" className="btn btn-outline-primary rounded-pill px-4">
                           ...
                           {d.status === 'pending' && !(d as any).is_paid && !user?.has_active_asd && (
                             <div className="d-flex flex-column gap-2">
                               ...
                               <button 
                                  ...
                                  onClick={() => navigate(`/paiement?amount=10.000&reason=${encodeURIComponent(d.service_type_label)}&requestId=${d.id}&requestType=eau&target=/mes-demandes-eau`)}
                               >
