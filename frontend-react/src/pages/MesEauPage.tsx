...
export default function MesEauPage() {
  ...
  return (
    <MainLayout user={user} onLogout={() => navigate('/login')} breadcrumbs={[{ label: 'Eau, Lumière & Égouts' }]}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="section-title text-primary text-uppercase fw-bold m-0">
            <i className="fas fa-tint me-2"></i>
            Mes demandes — Eau, Lumière & Égouts
          </h2>
          <p className="text-muted small">Historique de vos demandes de raccordement et réclamations compteurs.</p>
        </div>
        <Link to="/demande-eau" className="btn btn-primary rounded-pill shadow-sm">
                ...
                <tbody>
                  {demandes.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td className="text-muted small">{d.adresse}</td>
