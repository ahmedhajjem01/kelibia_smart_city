...
export default function MesImpotsPage() {
  ...
  return (
    <MainLayout user={user} onLogout={() => navigate('/login')} breadcrumbs={[{ label: 'Argent & Impôts' }]}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="section-title text-primary text-uppercase fw-bold m-0">
            <i className="fas fa-file-invoice-dollar me-2"></i>
            Mes demandes — Argent & Impôts
          </h2>
          <p className="text-muted small">Historique de vos demandes fiscales et immobilières.</p>
        </div>
        <Link to="/demande-argent" className="btn btn-primary rounded-pill shadow-sm">
              ...
              <table className="table table-hover align-middle mb-0">
                <thead className="bg-light">
                  <tr>
                    ...
                    <th className="border-0 py-3">Adresse du bien</th>
                ...
                <tbody>
                  {demandes.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td className="text-muted small">{d.adresse_bien}</td>
