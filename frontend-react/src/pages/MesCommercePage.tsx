...
export default function MesCommercePage() {
  ...
  return (
    <MainLayout
      ...
      breadcrumbs={[{ label: lang === 'ar' ? 'المحلات والتجارة' : 'Boutiques & Commerces' }]}
    >
      <div className={`py-4 ${lang === 'ar' ? 'text-end font-arabic' : ''}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div>
            <h1 className="fw-bold section-title mb-1">
              <i className={`fas fa-store ${lang === 'ar' ? 'ms-2' : 'me-2'}`}></i> {lang === 'ar' ? 'طلباتي — المحلات والتجارة' : 'Mes demandes — Boutiques & Commerces'}
            </h1>
            <p className="text-muted mb-0">{lang === 'ar' ? 'سجل طلبات الرخص التجارية الخاصة بك.' : 'Historique de vos demandes de licences commerciales.'}</p>
          </div>
          <Link to="/demande-commerce" className="btn btn-primary rounded-pill px-4 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="text-center py-5 card shadow-sm border-0 rounded-4">
            ...
            <Link to="/demande-commerce" className="btn btn-outline-primary rounded-pill px-4">
                  ...
                  {demandes.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td className="text-muted small">
                        {d.nom_commerce} — {d.adresse_commerce}
                      </td>
                           ...
                           {d.status === 'pending' && !(d as any).is_paid && !user?.has_active_asd && (
                             <div className="d-flex flex-column gap-2">
                               ...
                               <button 
                                  ...
                                  onClick={() => navigate(`/paiement?amount=15.000&reason=${encodeURIComponent(d.service_type_label)}&requestId=${d.id}&requestType=commerce&target=/mes-demandes-commerce`)}
                               >
                                  <i className="fas fa-credit-card me-1"></i> {lang === 'ar' ? `15.000 د.ت` : `15.000 DT`} {t('pay_label') || 'Payer'}
                               </button>
                             </div>
                           )}
                           {(d as any).is_paid && (
