...
export default function DemandeCommercePage() {
  ...
  const SERVICE_TYPES = [
    { value: 'licence_enseigne', label: lang === 'ar' ? 'ترخيص تركيز علامة إشهارية' : "Licence d'installation d'une enseigne publicitaire" },
  ]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(resolveBackendUrl('/api/commerce/demande/'), {
        method: 'POST',
  ...
  return (
    <MainLayout user={user} onLogout={() => navigate('/login')} breadcrumbs={[
      { label: lang === 'ar' ? 'المحلات والمتاجر' : 'Boutiques & Commerces', href: '/mes-commerce' },
          ...
          <div className="card shadow-sm border-0 rounded-4">
            <div className="card-header rounded-top-4 text-white py-3 px-4"
              ...
              <h4 className="mb-0 fw-bold">
                <i className={`fas fa-store ${lang === 'ar' ? 'ms-2' : 'me-2'}`}></i>
                {lang === 'ar' ? 'طلب — المحلات والمتاجر' : 'Demande — Boutiques & Commerces'}
              </h4>
              <p className="mb-0 small opacity-75">{lang === 'ar' ? 'التراخيص التجارية والعلامات الإشهارية' : 'Licences commerciales et enseignes publicitaires'}</p>
            </div>
            <div className="card-body p-4">
              {success ? (
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-3">
                    <label className="form-label fw-semibold">{lang === 'ar' ? 'اسم المحل' : 'Nom du commerce'} <span className="text-danger">*</span></label>
                    <input type="text" name="nom_commerce" className="form-control rounded-3"
                      ...
                      placeholder={lang === 'ar' ? 'مثال: عطارية بن صالح، صالون نور...' : "Ex: Épicerie Ben Salah, Salon Nour..."} />
                  </div>

                  <div className="mb-3">
                    <label className="form-label fw-semibold">{lang === 'ar' ? 'عنوان المحل' : 'Adresse du commerce'} <span className="text-danger">*</span></label>
                  ...
                  <div className="mb-3">
                    ...
                    <textarea name="description" className="form-control rounded-3" rows={3}
                      ...
                      placeholder={lang === 'ar' ? 'أبعاد العلامة، نوع العرض، المادة المستعملة...' : "Dimensions de l'enseigne, type d'affichage, matériau..."} />
                  </div>
                  ...
                  <div className="mb-4">
                    <label className="form-label fw-semibold">{lang === 'ar' ? 'صورة العلامة الإشهارية' : "Photo de l'enseigne"} <span className="text-muted small">({lang === 'ar' ? 'اختياري' : 'optionnel'})</span></label>
                    <div className="border rounded-3 p-3 text-center bg-light" style={{ cursor: 'pointer' }}
                      ...
                      {files.photo_enseigne
                        ? <><i className={`fas fa-check-circle text-success ${lang === 'ar' ? 'ms-2' : 'me-2'}`}></i><span className="text-success small">{files.photo_enseigne.name}</span></>
                        : <><i className="fas fa-image fa-2x text-muted mb-2 d-block"></i><span className="text-muted small">{lang === 'ar' ? 'صورة أو تصميم العلامة (اختياري)' : "Photo ou maquette de l'enseigne (optionnel)"}</span></>}
