...
const MASK_GEOJSON: GeoJSON.Feature<GeoJSON.Polygon> = {
  ...
  geometry: {
    type: 'Polygon',
    // First ring = world bounding box (outer), second ring = Kelibia boundary (hole)
    // GeoJSON hole must be in opposite winding order to the outer ring
    coordinates: [
      // World outer ring (counter-clockwise)
      [[-180, -90], [180, -90], [180, 90], [-180, 90], [-180, -90]],
      // Kelibia inner ring reversed = clockwise (creates the hole)
      ...
      <div style={{ background: '#fff', marginBottom: 28, border: '1px solid #eeeeee' }} id="mapCard">
        ...
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #eeeeee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {/* TEMP: mask toggle for report screenshots — delete after use */}
            <button
              onClick={() => setShowMask(m => !m)}
              title="Masquer hors commune (rapport)"
              style={{
        ...
        <div style={{ height: 360, position: 'relative' }}>
          ...
          {loadingMap ? (
            ...
            <MapContainer center={KELIBIA_CENTER} zoom={14} style={{ height: '100%', width: '100%' }}>
              ...
              {/* TEMP mask layer for report screenshots — delete after use */}
              {showMask && (
                <GeoJSON
                  key="mask"
