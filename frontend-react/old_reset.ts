...
export default function ResetPasswordConfirmPage() {
  ...
  const submitLabel = useMemo(() => {
    return loading ? 'Traitement...' : 'R├®initialiser'
  }, [loading])
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok || res.status === 204) {
        ...
        let errMsg = '├ëchec de la r├®initialisation.'
        ...
        if (data['new_password'] && Array.isArray(data['new_password'])) {
          ...
          errMsg = 'Le lien est invalide ou a expir├®.'
        }
      ...
      setMessage({
        type: 'danger',
        text: err instanceof Error ? err.message : 'Erreur de r├®initialisation.',
      })
    } finally {
      setLoading(false)
    }
